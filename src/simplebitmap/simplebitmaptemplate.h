...
public:
	...
	*/
	void CutOut(SimpleBitmapTemplate <ComponentType,NumComponentPerPixel> &destination,int thisX0,int thisY0,int wid,int hei,ComponentType clearColor) const
	{
		...
		int destBmpPtrCounter {0};

		for(int px_y = thisY0; px_y < thisY0+hei; ++px_y)
			for(int px_x = thisX0; px_x < thisX0+wid; ++px_x)	
				for(int i = 0; i < NumComponentPerPixel; ++i) 
				{
					if(IsInRange(px_x, px_y))
						*(destination.GetEditablePixelPointer(px_x-thisX0,px_y-thisY0) + i) = *(GetPixelPointer(px_x, px_y)+i);
					else
						*(destination.GetEditablePixelPointer(px_x-thisX0,px_y-thisY0) + i) = clearColor;
				}
	}
